...
import contract from "./contracts/labs.contract";
...
export function useLabs(hostTokenId, stimulusTokenId) {
  const { config } = usePrepareContractWrite({
    ...contract,
    functionName: "isFusionable",
    args: [hostTokenId, stimulusTokenId],
  });

  const { data, isLoading, isSuccess, write } = useContractWrite(config);

  return { data, isLoading, isSuccess, isFusionable: write };
}
